...
p2 <- ggplot(data=dat_3, aes(x=Size, y=Fraction.Dead, color=Crithidia)) + 
  geom_boxplot(outlier.shape=NA) + scale_color_manual(values=caPalette) +
  geom_point(position=position_jitterdodge(jitter.width = 0.3, jitter.height = 0.05)) + ylim(-0.05,1) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  xlab("Colony Size") +
   labs(y="Proportion Dead", tag = "(b)") + labs(color="Treatment") + 
...
FigureS1 <- ggplot(data=dat, 
  ...
  labs(y = "Proportion Dead", color="Treatment", linetype="Group Size") + 
...
# total weight
weight <- ggplot(dat, aes(x=Size, color=Inoculated, y=Nest_Weight)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) + ylim(0,80) +
  labs(tag = "(a)") +
  scale_color_manual(values = caPalette, labels = c("Control", "Inoculated")) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Total\n Nest Weight (g)", x="Colony Size") + labs(color="Treatment") + 
  cowplot::theme_cowplot()

# total reproductive output
fitness <- ggplot(dat, aes(x=Size, color=Inoculated, y=EggMale)) +
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) + 
               labs(color="Inoculated") + ylim(0,180) + labs(tag = "(b)") +
  scale_color_manual(values = caPalette, labels = c("Control", "Inoculated")) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Total\n No. Pupae + Males", x="Colony Size") + labs(color="Treatment") + 
  cowplot::theme_cowplot()

# per capita weight
percap_weight <- ggplot(dat, aes(x=Size, color=Inoculated, y=Per_Capita_Weight)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette) +
...
percap_eggmale <- ggplot(dat, aes(x=Size, color=Inoculated, y=Per_Capita_EggMale)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) + ylim(0,5.7) +
  labs(tag = "(d)") +
  scale_color_manual(values = caPalette, labels = c("Control", "Inoculated")) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_bw() + labs(y="Per Capita\n No. Pupae + Males", x="Colony Size") + labs(color="Treatment") + cowplot::theme_cowplot()


Figure2 <- ggarrange(weight,fitness,percap_weight,percap_eggmale,nrow=2,ncol=2)

ggsave(Figure2, file = "/Users/leah/Library/CloudStorage/OneDrive-CornellUniversity/Cornell/Density_Project/FIGURES/Fig2.tiff", 
...
honeypot <- ggplot(dat, aes(x=as.factor(Size), color=Inoculated, y=Honey_Pots)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette, labels = c("Control", "Inoculated")) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Total\n No. Honey Pots", x="Colony Size") + cowplot::theme_cowplot() + 
  labs(tag = "(e)", color="Treatment")

# Per capita number of honeypots
honeypot_percap <- ggplot(dat, aes(x=as.factor(Size), color=Inoculated, y=Per_Capita_HoneyPots)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette, labels = c("Control", "Inoculated")) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Per Capita\n No. Honey Pots", x="Colony Size") + cowplot::theme_cowplot() + 
  labs(tag = "(f)", color="Treatment")

##############################################################################################

# Pupae
# Total number of pupae
pupa <- ggplot(dat, aes(x=as.factor(Size), color=Inoculated, y=Egg_Cells)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette, labels = c("Control", "Inoculated")) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Total\n No. Pupae", x="Colony Size") + cowplot::theme_cowplot() + 
  labs(tag = "(a)", color="Treatment")

# Per capita number of pupae
pupa_percap <- ggplot(dat, aes(x=as.factor(Size), color=Inoculated, y=Per_Capita_Pupae)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette, labels = c("Control", "Inoculated")) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Per Capita\n No. Pupae", x="Colony Size") + cowplot::theme_cowplot() + 
  labs(tag = "(b)", color="Treatment")

################################################################################################

# Males
# Total number of males
male <- ggplot(dat, aes(x=as.factor(Size), color=Inoculated, y=Males)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette, labels = c("Control", "Inoculated")) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Total\n No. Males", x="Colony Size") + cowplot::theme_cowplot() + 
  labs(tag = "(c)", color="Treatment") 

# Per capita number of males 
male_percap <- ggplot(dat, aes(x=as.factor(Size), color=Inoculated, y=Per_Capita_Males)) + 
  geom_boxplot(outlier.shape = NA) + scale_color_manual(values=caPalette, labels = c("Control", "Inoculated")) +
  geom_point(position=position_jitterdodge(jitter.width=.25, jitter.height = 0.25)) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  theme_light() + labs(y="Per Capita\n No. Males", x="Colony Size") + cowplot::theme_cowplot() + 
  labs(tag = "(d)", color="Treatment") 
...
p1 <- ggplot(data=dat,
 ...
 geom_smooth(method="glm", method.args=list(family="binomial"),
  ...
  labs(y = "Proportion Infected", tag = "(a)", color="Group Size") + 
  scale_colour_manual(values=caPalette, labels = c("Small \n(15 bees)","Large \n(45 bees)")) + 
  theme(plot.title = element_text(face = "bold", size = 12)) +
  cowplot::theme_cowplot()

# Improved from first submission by adding jitter to make points clearly different
# Had to change yaxis slightly so jitter height wouldn't be outside bounds
p2 <- ggplot(data=dat_3, aes(x=Size, y=Infected_Prop, color=Size)) + 
  geom_boxplot(show.legend=FALSE, outlier.shape = NA) + scale_color_manual(values=caPalette) +
  geom_point(position=position_jitterdodge(jitter.width = 0.3, jitter.height = 0.05), show.legend = FALSE) + 
  scale_x_discrete(labels=c("Small\n (15 bees)\n", "Large\n (45 bees)")) + ylim(-0.05,1.05) +
  xlab("Group Size") +
  theme_light() + labs(y="Proportion Infected", tag = "(b)") + labs(color="Size") +
  cowplot::theme_cowplot()
...
FigureS3 <- ggplot(data=dat, 
  ...
  labs(y = "Proportion Infected", x = "Week after inoculation", color="Group Size") +
  scale_colour_manual(values=caPalette, labels = c("Small \n(15 bees)","Large \n(45 bees)")) + 
  theme_bw() + 
  theme(plot.title = element_text(face = "bold", size = 12), 
        panel.grid = element_blank())
...
p1 <- ggplot(data=dat_figures, aes(x=Size, y=Crithidia/0.5)) + geom_boxplot(aes(color=Size)) +
  cowplot::theme_cowplot() + ylab("Log No. Crithidia per uL") + xlab("Group Size") + labs(tag="(a)") +
...
p2 <- ggplot(data = dat_figures.all, aes(x=Size, y=Marginal_Cell)) + 
  ...
  xlab("Group Size") + ylab("Marginal Cell Length (mm)") + labs(tag="(c)") +
  scale_color_manual(values=caPalette) +
  scale_x_discrete(labels=c("Small\n (15 bees)", "Large\n (45 bees)")) +
  cowplot::theme_cowplot() +
  theme(legend.position = "none") 
